...
def author():
        return "kbarthelson3" # replace gburdell3 with your Georgia Tech username.

def gtid():
    return 903546171 # replace with your GT ID number
...
class PageRank:
    ...
    def calculate_node_degree(self):
        ...
        #############################################
            ...
            # look for the highest node degree (either source or target)
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        ### Implement your code here
        #############################################
                # (1-d)*Pd(v_j) when the target node is first found
                if new_pr_values[target] == 0:
                    new_pr_values[target] = (1-damping_factor)*node_weights[target]
                # d*PR_t(v_i)/out_degree(v_i) every time the target is found
